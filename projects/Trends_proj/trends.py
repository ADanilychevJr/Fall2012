...
def tweet_location(tweet):
    ...
    "*** YOUR CODE HERE ***"
    p = make_position(tweet['latitude'], tweet['longitude'])
    return p
...
def extract_words(text):
    ...
    length = len(text)
    while (j < length):
        if text[j] not in ascii_letters and text[i] in ascii_letters:
            lis.append(text[i:j])
            j+=1
            i = j
        elif text[i] not in ascii_letters:
            j+=1
            i+=1
        else:
            j+=1
    if (text[j-1] in ascii_letters):
        lis.append(text[i:length])
    return lis
...
def analyze_tweet_sentiment(tweet):
    ...
    for x in lst:
        if has_sentiment(get_word_sentiment(x)):
            total +=get_word_sentiment(x)
            i +=1
    if i == 0:
        return make_sentiment(None)
    else:
        av = total / i
        return make_sentiment(av)
...
def find_centroid(polygon):
    ...
    while count < len(polygon) - 1:
        Area += (latitude(polygon[count]) * longitude(polygon[count+1]) - latitude(polygon[count + 1]) * longitude(polygon[count]))
        count +=1
    Area = (Area / 2)
    if Area == 0:
    ...
    count = 0
    while count < len(polygon) - 1:
        cx += (latitude(polygon[count]) + latitude(polygon[count+1])) * ((latitude(polygon[count]) * longitude(polygon[count+1]) - latitude(polygon[count+1]) * longitude(polygon[count])))
        cy += (longitude(polygon[count]) + longitude(polygon[count+1])) * ((latitude(polygon[count]) * longitude(polygon[count+1]) - latitude(polygon[count+1]) * longitude(polygon[count])))
        count +=1
    cx = (cx / (6*Area))
    cy = (cy / (6*Area))
...
def find_center(polygons):
    ...
    count = 0
    for x in polygons:
        Asum += find_centroid(x)[2]
    while count < len(polygons):
        cxnum += find_centroid(polygons[count])[0] * find_centroid(polygons[count])[2]
        cynum += find_centroid(polygons[count])[1] * find_centroid(polygons[count])[2]
        count +=1
    cx = cxnum / Asum
    cy = cynum / Asum
...
def find_closest_state(tweet, state_centers):
    ...
    'NJ'
    """
    x = tweet_location(tweet)
    smallest = 10000000
    for y in state_centers:
        center = state_centers[y]
        if (geo_distance(x,center) < smallest):
            smallest = geo_distance(x,center)
            z = y
    return z
...
def average_sentiments(tweets_by_state):
    ...
    for x in tweets_by_state:
        total = 0
        count = 0
        for tweet in tweets_by_state[x]:
...
def group_tweets_by_hour(tweets):
    ...
    for x in range(0,24):
        tweets_by_hour[x] = []
    for x in tweets:
        hour = tweet_time(x).hour
        tweets_by_hour[hour].append(x)
