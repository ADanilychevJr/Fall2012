...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        '''frame = Frame(env)
        frame.bindings = dict(zip(procedure.formals, args))
        scheme_eval(procedure,frame)'''
        frame = Frame(procedure.env)
        #frame.bindings = dict(zip(procedure.formals, args))
        procedure2 = procedure.formals
        args2 = args
        while procedure2 and args2:
            
            frame.bindings[procedure2.first] = args2.first
            procedure2 = procedure2.second
...
def do_lambda_form(vals, env):
    ...
    if len(vals) < 3:
        #print(vals.second.first)
        x = LambdaProcedure(vals.first,vals.second.first,env)
        return x
    else:
        y = LambdaProcedure(vals.first,Pair('begin', vals.second),env)
        return y
...
def do_mu_form(vals):
    ...
    check_formals(formals)
    if len(vals) < 3:
        x = MuProcedure(vals.first,vals.second.first)
        return x
    else:
        y = MuProcedure(vals.first,Pair('begin',vals.second))
        return y
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        x = vals.first.first
        vals.first = vals.first.second
        env.bindings[x] = do_lambda_form(vals, env)
    else:
...
def do_let_form(vals, env):
    ...
    names, vals = nil, nil
    new_env = Frame(env)
    #print('forms', bindings.first.first)
    forms = Pair(bindings.first.first,nil)
    #print('values', bindings.first.second.first)
    values = Pair(bindings.first.second.first,nil)
    bindings = bindings.second
    while bindings:
           #print(bindings.first.first)
           forms.second = Pair(bindings.first.first,nil)
           #print(bindings.first.second.first)
           values.second = Pair(scheme_eval(bindings.first.second.first,new_env),nil)
           #print(values)
           bindings = bindings.second
    #new_env.make_call_frame(forms,values)
    #print(forms,values)
    #new_env = env.make_call_frame(names, vals)
    
    new_env = env.make_call_frame(forms,values)
    #print(new_env)
...
def do_and_form(vals, env):
    ...
    while len(vals)>0:
        if scheme_false(scheme_eval(vals[0], env)):
            return False
        vals=vals.second
        if len(vals) == 1 and scheme_true(scheme_eval(vals[0],env)):
            return vals[0]
    return len(vals)==0 or vals[0]
...
def do_or_form(vals, env):
    ...
    while len(vals)>1:
        k=vals[0]
        if scheme_true(scheme_eval(k, env)):
            return k
        vals=vals.second
        return len(vals)!=0 and vals[0]
...
def check_formals(formals):
    ...
    """
    dictionary = {}
    while not not formals:
        check_form(formals,0)
        if formals.first in dictionary:
            raise SchemeError("Repeated formal parameter")
        dictionary[formals.first] = 0
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if first in LOGIC_FORMS:
            if first=="and":
                expr=do_and_form(rest, env)
            elif first=="or":
                expr=do_or_form(rest, env)
            elif first=="begin":
                expr=do_begin_form(rest, env)
            elif first=="if":
                expr=do_if_form(rest, evn)
            ...
            expr, evn=do_let_form(rest, evn)
        else:
            pro=scheme_eval(first, env)
            args=rest.map(lambda operand:scheme_eval(operand, env))
            if isinstance(pro, LambdaProcedure):
                env=pro.env.make_call_frame(pro.formals, args)
                expr=pro.body
            elif isinstance(pro, PrimitiveProcedure):
                return apply_primitive(pro, args, env)
            elif isinstance(pro, MuProcedure):
                env=env.make_call_frame(pro.formals, args)
                expr=pro.body
            else:
                raise SchemeError("Error{0}".format(str(pro)))
