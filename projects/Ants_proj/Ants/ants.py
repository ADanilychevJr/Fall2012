...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            self.ant = insect.ant
        elif self.ant.name == 'Queen' and self.ant.first_queen:
            return None
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while b < c:
            a = a.entrance
            b+=1
        while a is not hive and a is not None:
            if b > self.max_range:
                return None
            if len(a.bees) != 0:
                x = len(a.bees)
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            if x > 0:
                y = random.random()
                z = int(y * x)
                self.eat_bee(self.place.bees[z])
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        # self.real = self.queen_count
        # QueenAnt.queen_count +=1
        self.true_queen = self.first_queen
    ...
    def action(self, colony):
        ...
        if not self.true_queen:        
            ...
            a = self.place.exit
            while a.name != 'AntQueen':
                if a.ant is not None:                
                    a.ant.damage = a.ant.__class__.damage * 2
                    if False and a.exit.ant.ant is not None:
                        a.exit.ant.ant.damage = a.exit.ant.ant.damage * 2
                a = a.exit
        self.throw_at(self.nearest_bee(colony.hive))
        colony.queen = QueenPlace(colony, self.place)
        colony.queen.ant=self
        
        
class QueenPlace(object):
        def __init__(self, colony, newplace):
            self.colony = colony.queen            
            self.new = newplace
        @property
        def bees(self):
            lst = self.colony.bees + self.new.bees
            return lst
